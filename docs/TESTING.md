...
| `PathBRoundTraceIsInert` | `SEMPER_PATHB_ROUNDS=1` turns on the Path B round-structure diagnostic without moving a single output float, metric count, Path A/B split or rescue tally. Wall-clock slots 9-14 are excluded, being timings | A diagnostic that perturbs what it measures is worse than none — and the GPU Phase 5 conclusion rests on numbers this diagnostic produced |
